        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title">Create New Sub-Subject</h3>
                ...
                <div class="card-body">
                    <form action="{{ route('admin.sub-subjects.store') }}" method="POST">
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control @error('sub_id') is-invalid @enderror" id="sub_id" name="sub_id" required>
                                ...
                                @foreach($subjects as $subject)
                                    <option value="{{ $subject->id }}" {{ old('sub_id') == $subject->id ? 'selected' : '' }}>{{ $subject->name }}</option>
                                @endforeach
                            </select>
                            @error('sub_id')
                        ...
                        <button type="submit" class="btn btn-primary">Create Sub-Subject</button>
                    </form>
                </div>
            </div>
        </div>
